...
The easiest path is to install [`gh`](https://cli.github.com) and run `gh auth login`. `gau` will pick up its token automatically. Alternatively, export a personal access token:
...
When running anonymously, `gau` prints:
