...
| `changes`  | Number of workflow files actually rewritten by this run. Always `0` when `write: false` or outside a git repository. When `commit: true`, counted from the just-created commit (`git diff HEAD~1..HEAD`); when `write: true` without `--commit`, counted from the working tree. Safe to gate downstream steps on, e.g. `if: steps.ghau.outputs.changes > 0`. |
